...
if [[ "${target}" == *"@"* ]]; then
    ...
    case "${target}" in
        *-openbsd*) bail "OpenBSD targets require specifying OS version by using \`${target}@<version>\` syntax because any minor version of OpenBSD can introduce changes that break ABIs" ;;
    esac
fi
